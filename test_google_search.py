...
def test_able_to_find_selene_in_google():
    search_for(text=selene_search_query,
               condition=have.text('User-oriented Web UI browser tests in Python'))


def test_unable_to_find_invalid_search_result_in_google():
    search_for(text=invalid_search_query, condition=have.no.text(invalid_search_query))
